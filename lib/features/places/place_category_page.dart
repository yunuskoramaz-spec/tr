...
class _PlaceCategoryPageState extends State<PlaceCategoryPage> {
  ...
  @override void dispose() { searchController.dispose(); mapController.dispose(); super.dispose(); }
  ...
  Widget build(BuildContext context) {
    final definition = definitions[widget.type] ?? (title: 'Diğer Yerler', icon: Icons.place_outlined, query: 'Kayseri');
    return PopScope(
      canPop: false,
      ...
      child: Scaffold(
        ...
        appBar: AppBar(
          ...
          title: Text(definition.title, style: const TextStyle(fontSize: 28, fontWeight: FontWeight.w500)),
        ...
        body: Column(children: [
          Container(
            ...
            child: Row(children: [
              Expanded(child: _ModeButton(active: !mapView, icon: Icons.list_alt, label: 'Liste', onTap: () => setState(() => mapView = false))),
              const SizedBox(width: 8),
              Expanded(child: _ModeButton(active: mapView, icon: Icons.map_outlined, label: 'Harita', onTap: () => setState(() => mapView = true))),
            ]),
          ),
          Padding(
          ...
          Expanded(child: loading
              ...
              : error != null
                  ...
                  : mapView ? _MapResults(results: results, icon: definition.icon, mapController: mapController, onDirections: directions)
                  : ListView.builder(
                      padding: const EdgeInsets.fromLTRB(16, 8, 16, 28),
                      itemCount: results.length,
                      itemBuilder: (_, i) => _PlaceCard(place: results[i], icon: definition.icon, onDirections: () => directions(results[i])),
                    )),
        ]),
      ),
    );
...
class _PlaceCard extends StatelessWidget {
  ...
  final PlaceResult place; final IconData icon; final VoidCallback onDirections;
  @override Widget build(BuildContext context) => Card(
    margin: const EdgeInsets.only(bottom: 12),
    shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(20), side: BorderSide(color: Colors.grey.shade200)),
    child: Padding(padding: const EdgeInsets.all(16), child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
      Row(crossAxisAlignment: CrossAxisAlignment.start, children: [
        Container(width: 52, height: 52, decoration: BoxDecoration(color: const Color(0xFFE5F8F3), borderRadius: BorderRadius.circular(16)), child: Icon(icon, color: const Color(0xFF2DBF9E), size: 28)),
        const SizedBox(width: 12),
        Expanded(child: Text(place.name, style: const TextStyle(fontSize: 20, fontWeight: FontWeight.w700))),
      ]),
      const SizedBox(height: 14),
      Row(crossAxisAlignment: CrossAxisAlignment.start, children: [const Icon(Icons.location_on_outlined, size: 22, color: Color(0xFF647A8F)), const SizedBox(width: 8), Expanded(child: Text(place.address, style: const TextStyle(fontSize: 15, color: Color(0xFF536B80), fontWeight: FontWeight.w600)))]),
      const SizedBox(height: 14),
      SizedBox(width: double.infinity, child: OutlinedButton.icon(onPressed: onDirections, icon: const Icon(Icons.directions_outlined), label: const Text('Yol Tarifi'), style: OutlinedButton.styleFrom(minimumSize: const Size.fromHeight(46), side: const BorderSide(color: Color(0xFF2F73D9)), foregroundColor: const Color(0xFF2F73D9), shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(14))))),
    ])),
  );
}
...
class _MapResults extends StatelessWidget {
  ...
  final List<PlaceResult> results; final IconData icon; final MapController mapController; final Future<void> Function(PlaceResult) onDirections;
  @override Widget build(BuildContext context) {
    final center = results.isNotEmpty ? LatLng(results.first.lat, results.first.lon) : const LatLng(38.7225, 35.4875);
    return Stack(children: [
      FlutterMap(
        ...
        children: [
          ...
          MarkerLayer(markers: results.map((p) => Marker(point: LatLng(p.lat, p.lon), width: 52, height: 52, child: GestureDetector(onTap: () => onDirections(p), child: Container(decoration: BoxDecoration(color: const Color(0xFF2DBF9E), shape: BoxShape.circle, border: Border.all(color: Colors.white, width: 3)), child: Icon(icon, color: Colors.white, size: 25))))).toList()),
        ],
      ),
